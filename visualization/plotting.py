...
def visualize_paths(gt_path, pred_path, html_tile="", title="VO exercises", file_out="plot.html"):
    ...
    frame_num_more_than_1m = len(diff)  # added by ola
    final_error = diff[-1]  
    ## added by ola
    # return the frame where the error exceeds a threshold of 1 meter
    for i in range(len(diff)):
        if diff[i] > 1:
            frame_num_more_than_1m = i
            break
    ...
    return frame_num_more_than_1m, max(diff), rmse, final_error
