...
from visualization.image import put_text, draw_matches
...
def play_trip(l_frames, r_frames=None, lat_lon=None, timestamps=None, color_mode=False, waite_time=100, win_name="Trip"):
    ...
    for i, frame_step in enumerate(zip(l_frames, r_frames)):
        ...
        if not isinstance(img_l, np.ndarray):
            print("in if")
            img_l = img_l.cpu().numpy()
...
## save the trip as a video
#def save_trip_video(l_frames, r_frames=None, lat_lon=None, timestamps=None, color_mode=False, waite_time=100, win_name="Trip"):
